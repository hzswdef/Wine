...
// Limits the tags to the specified count if true value specified on "limitTags" prop.
...
const Tags = memo(({ tags, appearance, ...props }: TagsProps) => {
	...
	useEffect(() => {
		...
		if (props.limitTags) {
			if (typeof props.limitTags === "boolean" && props.limitTags) {
				limitTags = defaultTagsLimit;
			} else {
